...
public class Picture extends SimplePicture 
{
	...
	for (int row = 156; row < mirrorPoint; row++)
		...
		for (int col = 100; col < 170; col++)
			...
			upPixel = pixels[row][col];      
			downPixel = pixels[mirrorPoint + mirrorPoint -row]                       
			    [col];
			downPixel.setColor(upPixel.getColor());
			count++;
		    }
	...
	// loop through the rows
	for (int row = 0; row < 363; row++)
	    {
		// loop from 13 to just before the mirror point
		for (int col = 155; col < mirrorPoint; col++)
		    {
    ...
    public void copyPart(Picture fromPic, 
			 int startRow, int startCol, int startPartRow, int startPartCol,
			 int endPartRow, int endPartCol)
    {
	Pixel fromPixel = null;
	Pixel toPixel = null;
	Pixel[][] toPixels = this.getPixels2D();
